...
const TaskProvider = ({ children }) => {
  ...
  const dragDrop = async (e, refetch) => {
    e.preventDefault();
    ...
    if (state === "trash") return handleDeleteTask(transferredId, refetch);
    ...
    if (changedStateTask.modifiedCount > 0) {
      ...
      xiosPublic
        ...
        .then((res) => {
          successToast(
            `${res.data.title} changed state into ${dragOverelement}`
          );
        })
        .catch((err) => {
          console.log(err);
