...
const CompletedTasks = ({ dragstarted }) => {
  const { user } = UseAuth();
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetData(`/tasks?email=${user?.email}&state=completed`, user?.email);
  ...
  return (
    <div>
      <div className="flex justify-center items-center ">
        {tasks?.length > 0 && <TitleDescription title={"Completed tasks"} />}
      ...
      <ul
        ...
        {tasks?.length > 0 ? (
          ...
          <div className="w-full h-full flex flex-col  justify-center">
            <h1 className="text-4xl mb-8 text-center mt-4">
