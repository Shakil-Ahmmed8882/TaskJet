...
const ToDoList = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetData(`/tasks?email=${user?.email}&state=to-do`, user?.email);
  ...
  return (
    <div>
      <div className="flex justify-center items-center ">
        {tasks?.length > 0 && <TitleDescription title={"To-Do tasks"} />}
      ...
      <ul
        ...
        {tasks?.length > 0 ? (
          ...
          <div className="w-full h-full flex  flex-col items-center  justify-center">
            <h1 className="text-4xl mt-6 mb-8">No To-Do task found</h1>
            <SkeletonListLoader />
            <SkeletonListLoader />
            <SkeletonListLoader />
          </div>
